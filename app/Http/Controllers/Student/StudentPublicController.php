...
// -----------------------
    ...
    // app/Http/Controllers/Student/StudentPublicController.php
   
    public function getSemesters($facultyId) {
        $semesters = FacultySemester::where('faculty_id', $facultyId)
                    ->join('semesters', 'faculty_semester.semester_id', '=', 'semesters.id')
                    ->select('semesters.id', 'semesters.semester')
                    ->get();
    ...
    public function register(AddValidation $request)
    {
        ...
        $semSec = $request->semester; // Use the semester directly from the form input

        // Rest of your existing code...
        $request->request->add(['semester' => $semSec]);
