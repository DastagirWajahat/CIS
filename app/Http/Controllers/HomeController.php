...
use App\Charts\StudentDepartmentChart;
use App\Charts\batchbarchart;
...
// ------------------------stdchart
            ...
            $data['studentDeptChart'] = new StudentDepartmentChart($departmentCounts);


//----------batch bar chart
            $batchCounts = Student::select('batch', DB::raw('COUNT(*) as count'))
            ->groupBy('batch')
            ->get()
            ->toArray();
            
            $data['batchbarchart'] = new batchbarchart($batchCounts);

        // Continue with existing code...s
